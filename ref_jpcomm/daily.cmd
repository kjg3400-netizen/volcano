@echo off
rem ---------------------------------------------------------------------
rem  Japanese community source hunter - scheduled runner
rem
...
set "LOG=ref_jpcomm\out\_log.txt"

if not exist "ref_jpcomm\out" mkdir "ref_jpcomm\out"

echo. >> "%LOG%"
echo ==== %DATE% %TIME% ==================================== >> "%LOG%"

"%PY%" ref_jpcomm\hunt.py --top 20 >> "%LOG%" 2>&1

if errorlevel 1 (
  echo [FAILED] hunt.py exit=%ERRORLEVEL% >> "%LOG%"
) else (
  echo [OK] see ref_jpcomm\out\ for the newest sheet >> "%LOG%"
  rem push the shortlist to the phone. never let this decide the job's exit code.
  "%PY%" ref_notify\notify_hunt.py --src jpcomm --top 5 >> "%LOG%" 2>&1
)
endlocal
